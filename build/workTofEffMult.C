...
TString to_string(double number);

void workTofEffMult(){

	file = new TFile("/gpfs01/star/pwg/mvranovsk/Run17_P20ic/TofEffMult_18.11.24/merged/StRP_production_0000.root");
...
TString to_string(double number) {
    stringstream ss;
    ss << number; // Insert the number into the stream
    string str = ss.str(); // Extract the string
...
void crystalBall(TH1D*& hist, const TString& outName, const TString& inputTag, bool is2TOF){
    ...
    vector<TString> names(j);
    vector<TObject*> obj(j);
    cout << "Frame objects:\n";
    j = 0;
