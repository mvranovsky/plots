...
void PlotV0SingleState::Init(){
    ...
    MCAna = new TFile("MCAnalysis.root");
...
void PlotV0SingleState::fitK0s(Double_t minRange, Double_t maxRange, int numBins, bool is2TOF) {
    ...
    signalFinal->Draw();
	int y = fitGaussPol2(&signalFinal,(maxRange-minRange)/numBins, minRange, maxRange);
    ...
    outFile->cd();
    if(is2TOF){
    	canvas->Write("invMassK0sFit2Canvas");
    	signalFinal->Write("invMassK0sFit2");
    }else{
    	canvas->Write("invMassK0sFit1Canvas");
...
void PlotV0SingleState::fitLambda(Double_t minRange, Double_t maxRange, int numBins, bool is2TOF) {
    ...
    signalFinal->Draw();
	int y = fitGaussPol2(&signalFinal,(maxRange-minRange)/numBins, minRange, maxRange, 0.01, 0.01, 0.01, 5000, 1.15, 0.01);
    ...
    outFile->cd();
    if(is2TOF){
    	canvas->Write("invMassLambdaFit2Canvas");
    	signalFinal->Write("invMassLambdaFit2");
    }else{
    	canvas->Write("invMassLambdaFit1Canvas");
...
vector<pair<int,double>> PlotV0SingleState::effFit(int Switch ,double Min, double Max,Double_t signalGuess1, Double_t signalGuess2, Double_t polGuess1[2], Double_t polGuess2[2]){
...
void PlotV0SingleState::efficiency(int switcher) {
	...
	for (int iBin = 0; iBin < binCount; ++iBin){ // considering 18 bins fro -0.9 to 0.9, skipping from -0.2 to 0.2 
		...
		/*
		if ( iBin == 4 && switcher == 1){  
			current_yield.clear();
			current_yield.push_back(make_pair(1,0));
			current_yield.push_back(make_pair(0,0));
			current_yield.push_back(make_pair(1,0));
			current_yield.push_back(make_pair(0,0));
		...
		current_yield = effFit(switcher,bottom + iBin*difference, bottom + difference*(1 + iBin),arg3[0], arg3[1], arg1, arg2); 
    ...
    TEfficiency *pEffMC = (TEfficiency*)MCAna->Get("eff" + variable);
    if(!pEffMC){
    	cout << "Couldn't load MC eff. Leaving" << endl;
    	return;
    }

    TGraphAsymmErrors *grMC = pEffMC->CreateGraph();
